...
final class Version20260211170000 extends AbstractMigration
{
    ...
    {
        return 'Ajouter medecin_id et patient_id à la table ordonnance si manquants.';
    }

    public function up(Schema $schema): void
    {
        $conn = $this->connection;
        $cols = $conn->createSchemaManager()->listTableColumns('ordonnance');
        ...
        if (!$hasMedecinId || !$hasPatientId) {
            $parts = [];
            if (!$hasMedecinId) {
                $parts[] = 'ADD medecin_id INT DEFAULT NULL';
            }
            if (!$hasPatientId) {
                $parts[] = 'ADD patient_id INT DEFAULT NULL';
            }
            $this->addSql('ALTER TABLE ordonnance ' . implode(', ', $parts));
        }
        if (!$hasMedecinId) {
